...
from .apply_linear_detector import apply_linear_detector

def detect_lei_enhanced(image: np.ndarray, zone_mask: np.ndarray, config: DefectDetectionConfig) -> np.ndarray:
    ...
    # Ensure masked_img is not all zeros before equalizing
    ...
    if scratch_strength.max() > 0:
        ...
        if masked_pixels.size > 0:
            ...
            for factor in config.lei_threshold_factors:
                ...
                _, mask = cv2.threshold(scratch_strength_uint8, threshold, 255, cv2.THRESH_BINARY)
...
if __name__ == '__main__':
    ...
    # Create a sample image with a scratch
    ...
    print("Saved 'lei_input.png' and 'lei_detection_mask.png' for visual inspection.")
