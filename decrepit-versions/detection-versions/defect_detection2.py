...
if __name__ == "__main__":
    # Assuming we have the results from mask_separation.py
    # Load the image and masks
    image_path = "test_fiber_image.jpg"
    image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    
    # Example masks (these would come from mask_separation.py)
    # For demonstration, creating dummy masks
    height, width = image.shape
    center = (width // 2, height // 2)
    
    # Create example masks
    inner_mask = np.zeros((height, width), dtype=bool)
    cv2.circle(inner_mask, center, 50, True, -1)
    
    annulus_mask = np.zeros((height, width), dtype=bool)
    cv2.circle(annulus_mask, center, 100, True, -1)
    cv2.circle(annulus_mask, center, 50, False, -1)
    
    # Initialize detector
    detector = ComprehensiveFiberDefectDetector()
    
    # Analyze defects in inner region
    results = detector.analyze_defects(image, inner_mask, region_type='inner')
    
    # Print summary
    print(f"Region Type: {results['region_type']}")
    ...
    # Visualize results
    detector.visualize_results('defect_analysis_results.png')
