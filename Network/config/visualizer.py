...
class FiberOpticsVisualizer:
    ...
    def __init__(self, use_interactive: bool = False):
        ...
        # Fixed: Use colors from config.yaml normalized to [0,1] instead of hard-coded values
        # Why: To ensure consistency with the configuration file and allow customization without code changes
        self.region_colors = {k: [c / 255.0 for c in v] for k, v in self.config.visualization.segmentation_colors.items()}
        
        # Fixed: Use anomaly colormap from config.yaml instead of hard-coded 'hot'
        # Why: To make the colormap configurable and match the system's visualization settings
        self.anomaly_cmap = getattr(plt.cm, self.config.visualization.anomaly_colormap)
    ...
    def _create_segmentation_overlay(self, image: np.ndarray, 
        ...
        try:
            ...
            # Added check to limit to min(num_classes, len(region_names)) to prevent IndexError if model num_classes > available colors in config.
...
if __name__ == "__main__":
    ...
    visualizer.visualize_complete_analysis(dummy_image, dummy_results, str(Path(__file__).parent.parent / 'test_visualization.png'))
    
    # Test configuration overview visualization
    logger.info("Testing configuration overview visualization...")
    visualizer.visualize_config_overview(str(Path(__file__).parent.parent / 'config_overview.png'))
    
    # Test equation components visualization
    logger.info("Testing equation components visualization...")
    visualizer.visualize_equation_components(dummy_results, str(Path(__file__).parent.parent / 'equation_components.png'))
    
    # Test configuration report generation
    logger.info("Testing configuration report generation...")
    visualizer.generate_config_report(str(Path(__file__).parent.parent / 'config_report.html'))
    ...
    visualizer.visualize_config_diff(test_config, ref_config, str(Path(__file__).parent.parent / 'config_diff.png'))
