...
class MultiScaleFeatureExtractor(nn.Module):
    ...
    def __init__(self):
        ...
        for i, out_channels in enumerate(self.config.FEATURE_CHANNELS):
        ...
        for i in range(len(self.config.FEATURE_CHANNELS) - 1):
            correlator = nn.Conv2d(
                self.config.FEATURE_CHANNELS[i] + self.config.FEATURE_CHANNELS[i + 1],
                self.config.FEATURE_CHANNELS[i + 1],
                1
            )
        ...
        self.gradient_weight_net = nn.Sequential(
            nn.Linear(1, 16),
            nn.ReLU(),
            nn.Linear(16, len(self.config.FEATURE_CHANNELS)),
            nn.Sigmoid()
        )
