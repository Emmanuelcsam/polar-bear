...
from similarity import CombinedSimilarityMetric
# from fiber_real_time_optimization import AdaptiveComputationModule
from config_loader import get_config as get_advanced_config
    ...
    def _init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d)):
                # Check if weight is initialized before accessing
                if hasattr(m, 'weight') and m.weight is not None and m.weight.shape:
                    try:
                        nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    except RuntimeError:
                        pass  # Skip if already initialized or has issues
