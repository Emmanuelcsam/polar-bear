...
class ConfigLoader:
    ...
    def get_region_folders(self, region: str) -> List[str]:
        ...
        folders = self.REGION_CATEGORIES.get(region, [])
        if not folders:
            print(f"[{datetime.now()}] WARNING: No folders found for region '{region}' in REGION_CATEGORIES")
        return folders  # Added warning log; original returned empty list silently, which could lead to downstream errors (e.g., empty datasets in data_loader.py) without notification
...
class ConfigManager:
    ...
    def initialize(self, config_path: str = "config/config.yaml"):
