...
class SAM(Optimizer):
    ...
    def __init__(self, params, base_optimizer, rho=0.05, adaptive=True, **kwargs):
        ...
        # Initialize base optimizer with parameters
        ...
        super(SAM, self).__init__(params, defaults=dict(rho=rho))
    ...
    def step(self, closure=None):
        ...
        Requires gradient computation twice: at current point and perturbed point
        """
        ...
        for idx, (group, slow_group) in enumerate(zip(
            ...
            for p_idx, (p, p_slow) in enumerate(zip(group['params'], slow_group)):
                ...
                if self.pullback_momentum == "pullback":
                    ...
                    p.data.copy_(p_slow.data.add_(buf))
                else:
                    # Standard update: θ_s + α(θ_f - θ_s)
                    p.data.mul_(self.alpha).add_(p_slow.data, alpha=1-self.alpha)
                
                # Update slow weights reference
                p_slow.data.copy_(p.data)
