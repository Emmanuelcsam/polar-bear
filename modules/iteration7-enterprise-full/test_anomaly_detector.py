...
class TestAnomalyDetector(unittest.TestCase):
    ...
    def test_anomaly_detection_accuracy(self):
        ...
        np.random.seed(42)
        # Normal data centered around 128
        normal_data = [int(x) for x in np.random.normal(128, 10, 96)]
        # Add extreme outliers that should definitely be detected
        outliers = [0, 0, 255, 255]
        pixels = normal_data + outliers
        ...
        self.assertGreater(len(anomalies['z_score_anomalies']) + len(anomalies['iqr_anomalies']), 0)
        
        # Check if extreme values are in the detected anomalies
        all_anomaly_values = []
        all_anomaly_values.extend([a['value'] for a in anomalies['z_score_anomalies']])
        all_anomaly_values.extend([a['value'] for a in anomalies['iqr_anomalies']])
        
        # At least one of 0 or 255 should be detected as anomaly
        self.assertTrue(0 in all_anomaly_values or 255 in all_anomaly_values)
