...
from .find_anomaly_regions import find_anomaly_regions
from .detect_specific_defects import detect_specific_defects
from skimage.metrics import structural_similarity

def _compute_local_anomaly_map(test_img: np.ndarray, reference_img: np.ndarray) -> np.ndarray:
    """Compute local anomaly map using sliding window and SSIM."""
    h, w = test_img.shape
    anomaly_map = np.zeros((h, w), dtype=np.float32)
    
    for win_size in [16, 32, 64]:
        if h < win_size or w < win_size: continue
        stride = win_size // 2
...
def detect_anomalies_comprehensive(test_path: str, knowledge_base_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"✗ Knowledge base not found at {knowledge_base_path}. Build one first."); return None

    test_image, metadata = load_image(test_path)
    if test_image is None: return None
    ...
    test_vector = np.array([test_features.get(fname, 0) for fname in ref_model['feature_names']])
    ...
    is_anomalous = mahalanobis_dist > thresholds.get('anomaly_threshold', 2.5) or structural_comp['ssim'] < 0.7
    
    return {
        'test_image': test_image, 'metadata': metadata,
        'global_analysis': {'mahalanobis_distance': float(mahalanobis_dist)},
        'structural_analysis': structural_comp,
        'local_analysis': {'anomaly_map': anomaly_map, 'anomaly_regions': anomaly_regions},
        'specific_defects': detect_specific_defects(test_gray),
        'verdict': {'is_anomalous': is_anomalous, 'confidence': min(1.0, mahalanobis_dist / thresholds.get('anomaly_threshold', 2.5))},
    }

if __name__ == '__main__':
    print("This script contains the main 'detect_anomalies_comprehensive' function.")
    ...
    if not Path(kb_path).exists():
        ...
        if results:
            print("\nAnalysis Complete!")
            print(f"  Verdict: {'ANOMALOUS' if results['verdict']['is_anomalous'] else 'NORMAL'}")
            ...
            print(f"  Anomaly Regions Found: {len(results['local_analysis']['anomaly_regions'])}")
