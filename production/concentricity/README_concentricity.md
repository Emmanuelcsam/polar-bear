...
The concentricity tester goes beyond simple circle fitting by analyzing:
...
### 1. `concentricity_tester.py`
The core analysis module containing:
- `ConcentricityTester`: Main class that performs multi-factor circularity analysis
- `ConcentricityProcessor`: High-level processor combining Hough detection with analysis

### 2. `concentricity_gui.py`
...
### Scoring System
The overall concentricity score (0-100%) is a weighted combination of:
...
### Programmatic Usage
```python
from concentricity_tester import ConcentricityTester
import cv2
...
# Analyze concentricity
results = tester.analyze_concentricity(image, circle)

# Display results
print(f"Concentricity Score: {results['concentricity_score']:.1f}%")
...
```python
from concentricity_tester import ConcentricityProcessor
...
processor = ConcentricityProcessor(hough_detector=detector)
...
### Custom Weighting
Modify the weights in `_calculate_concentricity_score()` to emphasize different aspects:
```python
weights = {
