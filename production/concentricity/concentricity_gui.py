"""
Concentricity Tester GUI Application
Real-time video processing with concentricity analysis using Hough circles
...
from concentricity_tester import ConcentricityTester, ConcentricityProcessor


class ConcentricityVideoDisplay:
    """
    Enhanced video display widget that shows frames with concentricity analysis.
    """
    ...
    def __init__(self, parent, width=640, height=480):
        ...
        # Concentricity processor
        self.concentricity_processor = ConcentricityProcessor()
    ...
    def update_frame(self, frame):
        """Update the display with a new frame and perform concentricity analysis."""
        if frame is None:
            self.info_label.config(text="No frame available")
            return
        
        try:
            # Process frame with concentricity analysis
            processed_frame, results = self.concentricity_processor.process_frame(frame)
            ...
            if results and 'concentricity_score' in results:
                score = results['concentricity_score']
                status = f"Frame: {width}x{height} | Concentricity: {score:.1f}%"
...
class ConcentricityAnalysisPanel:
    """
    Panel showing detailed concentricity analysis metrics.
    """
    ...
    def __init__(self, parent):
        ...
        # Create frame
        self.frame = ttk.LabelFrame(parent, text="Concentricity Analysis", padding="5")
    ...
    def update_metrics(self, results):
        ...
        score = results.get('concentricity_score', 0)
...
class ConcentricityPlotPanel:
    """
    Panel for displaying live plots of concentricity analysis.
    """
    ...
    def _initialize_plots(self):
        ...
        self.ax2.set_title('Concentricity Score History')
...
class ConcentricityGUI:
    """
    Main GUI application for the concentricity tester.
    """
    
    def __init__(self, root):
        self.root = root
        self.root.title("Concentricity Tester - Real-time Analysis")
    ...
    def _create_widgets(self):
        ...
        # Analysis panel
        self.analysis_panel = ConcentricityAnalysisPanel(left_frame)
        ...
        self.video_display = ConcentricityVideoDisplay(video_frame, width=640, height=480)
        
        # Right panel (plots)
        right_frame = ttk.Frame(main_paned)
        main_paned.add(right_frame, weight=2)
        
        self.plot_panel = ConcentricityPlotPanel(right_frame)
    ...
    def _update_hough_params(self, event=None):
        ...
        if self.video_display:
            try:
                detector = self.video_display.concentricity_processor.hough_detector
        ...
        if filename:
            try:
                with open(filename, 'w') as f:
                    f.write("Concentricity Analysis Results\n")
                    f.write("=" * 50 + "\n\n")
                    f.write(f"Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                    f.write(f"Image: {self.image_path_var.get()}\n\n")
                    
                    f.write("Overall Metrics:\n")
                    f.write(f"  Concentricity Score: {results.get('concentricity_score', 0):.1f}%\n")
