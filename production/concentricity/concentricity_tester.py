...
class ConcentricityTester:
    """
    Analyzes the concentricity/circularity of objects in images by comparing
    ...
    def __init__(self, 
        ...
        """
        Initialize the concentricity tester.
        ...
        # Statistics
        self.last_concentricity_score = 0.0
        self.last_analysis_results = {}
        
    def analyze_concentricity(self, frame: np.ndarray, circle: Tuple[int, int, int]) -> Dict:
        """
        Analyze the concentricity of an object given a Hough circle fit.
        ...
        Returns:
            Dictionary containing:
                - concentricity_score: Overall score (0-100%)
        ...
        # Calculate overall concentricity score
        concentricity_score = self._calculate_concentricity_score(
            radial_profile, intensity_profile, gradient_analysis, shape_metrics
        )
        
        results = {
            'concentricity_score': concentricity_score,
        ...
        self.last_concentricity_score = concentricity_score
    ...
    def _calculate_concentricity_score(self, radial_profile: Dict, intensity_profile: Dict,
                                     gradient_analysis: Dict, shape_metrics: Dict) -> float:
        """
        Calculate overall concentricity score from all analyses.
        """
    ...
    def visualize_analysis(self, frame: np.ndarray, results: Dict) -> np.ndarray:
        """
        Create visualization of the concentricity analysis.
        """
        if not results or results['concentricity_score'] == 0:
            return frame
        ...
        # Add text
        y_offset = 30
        cv2.putText(output, f"Concentricity: {results['concentricity_score']:.1f}%",
        ...
        # Color code the circle based on concentricity
        score = results['concentricity_score']
        if score > 90:
    ...
    def plot_detailed_analysis(self, results: Dict) -> None:
        ...
        fig.suptitle(f'Concentricity Analysis - Score: {results["concentricity_score"]:.1f}%')
...
class ConcentricityProcessor:
    """
    High-level processor that combines Hough circle detection with concentricity analysis.
    """
    
    def __init__(self, hough_detector=None, concentricity_tester=None):
        """
        Initialize the processor.
        
        Args:
            hough_detector: HoughCirclesDetector instance
            concentricity_tester: ConcentricityTester instance
        """
        # Import here to avoid circular dependency
        from hough_circles import HoughCirclesDetector
        
        self.hough_detector = hough_detector or HoughCirclesDetector()
        self.concentricity_tester = concentricity_tester or ConcentricityTester()
        self.processing_enabled = True
        self.visualization_mode = 'full'  # 'full', 'simple', 'none'
        
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict]:
        """
        Process a frame with both circle detection and concentricity analysis.
        ...
        if circles is None or len(circles) == 0:
            return hough_frame, {'concentricity_score': 0, 'message': 'No circles detected'}
        
        # Analyze the most prominent circle (first one returned by Hough)
        circle = circles[0]  # (x, y, radius)
        
        # Perform concentricity analysis
        results = self.concentricity_tester.analyze_concentricity(frame, circle)
        
        # Create visualization based on mode
        if self.visualization_mode == 'full':
            output_frame = self.concentricity_tester.visualize_analysis(frame, results)
        elif self.visualization_mode == 'simple':
            output_frame = hough_frame
            # Add concentricity score
            cv2.putText(output_frame, f"Concentricity: {results['concentricity_score']:.1f}%",
    ...
    def toggle_processing(self) -> bool:
        ...
        logging.info(f"Concentricity processing {'enabled' if self.processing_enabled else 'disabled'}")
...
def main():
    """
    Example usage and testing of the concentricity tester.
    """
    import sys
    ...
    # Create tester
    tester = ConcentricityTester()
    
    # Analyze
    results = tester.analyze_concentricity(test_image, (250, 250, 98))
    
    print("Concentricity Analysis Results:")
    print(f"Overall Score: {results['concentricity_score']:.1f}%")
    ...
    cv2.imshow('Concentricity Test', output)
    cv2.waitKey(0)
