...
class OmniFiberAnalyzer:
    ...
    def _convert_to_pipeline_format(self, results: Dict, image_path: str) -> Dict:
        ...
        for dig in specific_defects['digs']:
            ...
            cx, cy = dig['center']
            # Calculate approximate radius from area (A = Ï€rÂ²)
    ...
    def _compute_entropy(self, data, bins=256):
        ...
        hist = hist[hist > 0]
        # Compute Shannon entropy: -Î£(p * log2(p))
    ...
    def compute_exhaustive_comparison(self, features1, features2):
        ...
        comparison['chebyshev_distance'] = float(np.max(np.abs(vec1 - vec2)))     # Lâˆž distance
        ...
        if bins > 2:
            ...
            # Chi-square distance: Ï‡Â² = 0.5 * Î£ (P(i) - Q(i))Â² / (P(i) + Q(i))
    ...
    def visualize_comprehensive_results(self, results, output_path):
        ...
        ax7.axvline(x=2, color='orange', linestyle='--', alpha=0.5, label='2Ïƒ threshold')
        ax7.axvline(x=3, color='red', linestyle='--', alpha=0.5, label='3Ïƒ threshold')
        ax7.legend()
...
Global Analysis:
â€¢ Mahalanobis Distance: {global_stats['mahalanobis_distance']:.2f}
â€¢ Max Comparison Score: {global_stats['comparison_stats']['max']:.3f}
â€¢ Mean Comparison Score: {global_stats['comparison_stats']['mean']:.3f}

Structural Analysis:
â€¢ SSIM Index: {structural['ssim']:.3f}
â€¢ Mean Luminance Similarity: {structural['mean_luminance']:.3f}
â€¢ Mean Contrast Similarity: {structural['mean_contrast']:.3f}
â€¢ Mean Structure Similarity: {structural['mean_structure']:.3f}

Local Analysis:
â€¢ Anomaly Regions Found: {len(results['local_analysis']['anomaly_regions'])}
â€¢ Max Region Confidence: {max([r['confidence'] for r in results['local_analysis']['anomaly_regions']], default=0):.3f}

Criteria Triggered:
â€¢ Mahalanobis: {'âœ“' if verdict['criteria_triggered']['mahalanobis'] else 'âœ—'}
â€¢ Comparison: {'âœ“' if verdict['criteria_triggered']['comparison'] else 'âœ—'}
â€¢ Structural: {'âœ“' if verdict['criteria_triggered']['structural'] else 'âœ—'}
â€¢ Local: {'âœ“' if verdict['criteria_triggered']['local'] else 'âœ—'}"""
