...
class Config:
    ...
    def configure_interactively(self):
        ...
        print("  1. Auto (try all methods, prefer silent)")
        print("  2. MSS (fastest, SILENT - recommended)")
        print("  3. PyScreenshot (compatible, can be silent)")
        print("  4. PyAutoGUI (reliable but may flash)")
        method = input("Choose capture method [2]: ").strip()
        if method == '3':
            ...
            self.capture_method = 'mss'  # Default to MSS for silence
...
class ScreenCapture:
    """Multi-method screen capture with fallbacks - SILENT mode"""
    ...
    def __init__(self, method='auto'):
        ...
        # Disable PyAutoGUI screenshot sound/flash
        try:
            import pyautogui
            # Disable screenshot notifications
            pyautogui.screenshot = self._silent_pyautogui_screenshot
        except:
            pass
    ...
    def _silent_pyautogui_screenshot(self):
        """Silent screenshot using PIL directly"""
        try:
    ...
    def _find_working_method(self):
        """Find a working capture method - prioritize silent methods"""
        # Prioritize silent methods first
        methods = ['mss', 'pyscreenshot', 'pyautogui']
        
        for method in methods:
            if self._test_method(method):
                self.working_method = method
                logger.info(f"Using SILENT screen capture method: {method}")
                break
        else:
    ...
    def capture(self) -> Optional[np.ndarray]:
        """Capture screen using the working method - SILENT"""
        if not self.working_method:
            return None
            
        try:
            if self.working_method == 'mss':
                # MSS is silent by default
                if not self.sct:
                    self.sct = mss.mss()
                screenshot = self.sct.grab(self.sct.monitors[0])
                img = np.array(screenshot)
                ...
                # Use MSS backend for pyscreenshot to avoid flashing
                try:
                    import pyscreenshot
                    # Force MSS backend which is silent
                    img = pyscreenshot.grab(backend='mss')
                    img_np = np.array(img)
                    return cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
                except:
                    # Fallback to default but still try
                    img = pyscreenshot.grab()
                    img_np = np.array(img)
                    return cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
                
            elif self.working_method == 'pyautogui':
                # PyAutoGUI with our silent wrapper
                img = pyautogui.screenshot()
                img_np = np.array(img)
                return cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
...
def main():
    ...
    print("=" * 60)
    print("✨ SILENT MODE: No screen flashing or sounds during capture")
    print("=" * 60)
