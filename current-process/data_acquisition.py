...
class DefectAggregator:
    ...
    def load_separation_masks(self):
        """
        Load region masks and metadata from disk for each variation.
        If ram_mode == True: skips all disk loading (assumes in-memory masks already populated).
        """
        ram_mode = self.config.get('ram_mode', False)
        if ram_mode:
            self.logger.info("RAM mode enabled, skipping mask file loading.")
            return

        sep_root = self.results_dir / '2_separated'
        self.logger.info(f"Loading masks from {sep_root}")

        for variation_dir in sep_root.iterdir():
            if not variation_dir.is_dir():
                continue
            report = variation_dir / 'consensus_report.json'
            if not report.exists():
                continue
            data = json.load(open(report))
            center = data.get('center', [0,0])
            core_r = data.get('core_radius', 0)
            clad_r = data.get('cladding_radius', 0)
            offset_x = center[0] - (self.width / 2)
            offset_y = center[1] - (self.height / 2)

            for region in ['core', 'cladding', 'ferrule']:
                mask_file = variation_dir / f"region_{region}.png"
                if not mask_file.exists():
                    continue
                img = cv2.imread(str(mask_file), cv2.IMREAD_GRAYSCALE)
                mask = (img > 0).astype(np.uint8)
                key = f"{variation_dir.name}_region_{region}"
                self.region_masks[key] = mask

                ys, xs = np.where(mask)
                if xs.size and ys.size:
                    self.region_offsets[key] = {
                        'x_offset': int(xs.min()),
                        'y_offset': int(ys.min()),
                        ...
                        'core_radius': core_r,
                        'cladding_radius': clad_r
                    }
                    
    def map_defect_to_global_coords(self, defect):
        """
        Convert per-region defect (x,y) into global image coords using stored offsets.
        """
        x_loc, y_loc = defect['location_xy']
        if not defect.get('is_region'):
            return int(x_loc), int(y_loc)

        source = defect.get('source_image')  # e.g. 'variation1_region_core'
        offsets = self.region_offsets.get(source)
        if not offsets:
            # fallback
            return int(x_loc), int(y_loc)

        return (
            int(x_loc + offsets['x_offset']),
            int(y_loc + offsets['y_offset'])
        )
    ...
    def run_complete_analysis(self) -> Dict:
        ...
        try:
            ...
            # Always create final overlay visualization
            final_overlay_path = output_dir / f"{base_name}_FINAL_OVERLAY.png"
            self.create_final_overlay_visualization(merged_defects, final_overlay_path)     
    ...
    def create_final_overlay_visualization(self, merged_defects, output_path):
        """
        Draw each defect on the original image, color‑coded by type.
        Also logs region metrics (core/cladding sizes and offsets) and
        produces a two-panel figure: overlay and legend.

        - Overlay is the original RGB image with colored markers:
          • Blobs: green squares
          • Scratches: red diagonal lines
          • Contamination: yellow circles
        - Legend panel shows mapping of colors to defect types.
        """
        # Convert BGR to RGB for plotting
        img_rgb = cv2.cvtColor(self.original_image, cv2.COLOR_BGR2RGB)

        # Log core/cladding radii and center offsets for all variations
        for source, offs in self.region_offsets.items():
            core_r = offs.get('core_radius')
            clad_r = offs.get('cladding_radius')
            center_off = offs.get('center_offset')
            self.logger.info(
                f"Region {source}: core_radius={core_r}, \
                 cladding_radius={clad_r}, center_offset={center_off}"
            )

        # Define normalized colors for defect types
        type_colors = {
            'BLOB': (0, 1, 0),          # green
            'SCRATCH': (1, 0, 0),       # red
        ...
        # Create figure with two panels: overlay + legend
        fig, (ax_overlay, ax_legend) = plt.subplots(1, 2, figsize=(18, 8), gridspec_kw={'width_ratios': [3, 1]})
        ax_overlay.imshow(img_rgb)
        ax_overlay.axis('off')
        ax_overlay.set_title('Defects Overlay')

        # Plot each defect
        for defect in merged_defects:
            x, y = defect['global_location']
            d_type = defect.get('defect_type', 'UNKNOWN').upper()
            color = type_colors.get(d_type, type_colors['UNKNOWN'])

            if d_type == 'SCRATCH':
                # draw diagonal scratch line
                line = plt.Line2D([x-10, x+10], [y-10, y+10], linewidth=2, color=color)
            ...
            elif d_type == 'CONTAMINATION':
                # draw circle
                circ = patches.Circle((x, y), radius=6,
                                      linewidth=2, edgecolor=color,
                                      facecolor='none')
                ax_overlay.add_patch(circ)
            else:
                # fallback marker
                ax_overlay.plot(x, y, marker='x', color=color, markersize=8)

        # Build legend entries
        legend_handles = []
        for dtype, col in type_colors.items():
            if dtype == 'SCRATCH':
                handle = plt.Line2D([0], [0], color=col, lw=2)
            elif dtype == 'BLOB':
                handle = patches.Rectangle((0,0), 1, 1, edgecolor=col, facecolor='none', lw=2)
            elif dtype == 'CONTAMINATION':
                handle = patches.Circle((0,0), radius=0.5, edgecolor=col, facecolor='none', lw=2)
            else:
                handle = plt.Line2D([0], [0], marker='x', color=col, linestyle='')
            legend_handles.append(handle)
        ax_legend.legend(legend_handles, list(type_colors.keys()), loc='center')
        ax_legend.axis('off')
