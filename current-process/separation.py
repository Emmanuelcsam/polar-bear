...
class EnhancedConsensusSystem:
    ...
    def ensure_mask_consistency(self, core_mask: np.ndarray, cladding_mask: np.ndarray, ferrule_mask: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Post‑process masks to ensure consistency and clean boundaries.
        """
        ...
        if HAS_SCIPY_FULL:
            ...
            core_mask = binary_opening(core_mask, structure=structure)
            core_mask = binary_closing(core_mask, structure=structure)
        else:
            core_mask = cv2.morphologyEx(core_mask, cv2.MORPH_OPEN, kernel, iterations=1)
            core_mask = cv2.morphologyEx(core_mask, cv2.MORPH_CLOSE, kernel, iterations=1)

        # Process cladding mask
        if HAS_SCIPY_FULL:
            cladding_mask = binary_opening(cladding_mask, structure=structure)
            cladding_mask = binary_closing(cladding_mask, structure=structure)
        else:
            cladding_mask = cv2.morphologyEx(cladding_mask, cv2.MORPH_OPEN, kernel, iterations=1)
            cladding_mask = cv2.morphologyEx(cladding_mask, cv2.MORPH_CLOSE, kernel, iterations=1)

        # Process ferrule mask
        if HAS_SCIPY_FULL:
            ferrule_mask = binary_opening(ferrule_mask, structure=structure)
            ferrule_mask = binary_closing(ferrule_mask, structure=structure)
        else:
            ferrule_mask = cv2.morphologyEx(ferrule_mask, cv2.MORPH_OPEN, kernel, iterations=1)
            ferrule_mask = cv2.morphologyEx(ferrule_mask, cv2.MORPH_CLOSE, kernel, iterations=1)

        return core_mask.astype(np.uint8), cladding_mask.astype(np.uint8), ferrule_mask.astype(np.uint8)
...
class UnifiedSegmentationSystem:
    ...
    def process_ndarray(self, image_array: np.ndarray, virt_name: str, output_dir_str: str) -> Optional[Dict]:
        """
        ...
        try:
            res = self.process_image(Path(tmp.name), output_dir_str)
...
"""
    ...
    def process_image(self, image_path: Path, output_dir_str: str) -> Optional[Dict]:
        ...
        cv2.imwrite(str(tmp_path), inpainted_img)

        # Run each method
        all_results = []
        for method_name in self.methods:
            use_img = tmp_path if method_name in self.vulnerable_methods else image_path
            result = self.run_method(method_name, use_img, image_shape)
            all_results.append(result)

        # Consensus
        consensus = self.consensus_system.generate_consensus(
            all_results,
            {m: info['score'] for m, info in self.methods.items()},
            image_shape
        )
        ...
        with open(result_dir / "consensus_report.json", 'w') as f:
            json.dump({k:v for k,v in consensus.items() if k!='all_results'},
                      f, indent=4, cls=NumpyEncoder)

        # Masks
        masks = consensus['masks']
        core_path     = result_dir / "region_core.png"
        cladding_path = result_dir / "region_cladding.png"
        ferrule_path  = result_dir / "region_ferrule.png"
        cv2.imwrite(str(core_path),     masks['core'] * 255)
        cv2.imwrite(str(cladding_path), masks['cladding'] * 255)
        cv2.imwrite(str(ferrule_path),  masks['ferrule'] * 255)
        cv2.imwrite(str(result_dir / "detected_defects.png"), defect_mask)

        # Provide saved_regions for app.py
        consensus['saved_regions'] = [
            str(core_path),
            str(cladding_path),
            str(ferrule_path)
        ]
