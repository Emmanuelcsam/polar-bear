...
def setup_tracker():
    ...
    # Create tools directory or use current directory
    config = ConfigLoader()
    
    print("
Where are the tracker tools located?")
    print("1. Current directory")
    print("2. In 'project-tracker-tools' subdirectory")
...
Your choice (1-3): ").strip()
    ...
    if choice == '1':
        ...
        custom_path = input("Enter tools directory path: ").strip()
        tools_dir = Path(custom_path).expanduser().resolve()
        if not tools_dir.exists():
            print(f"❌ Directory does not exist: {tools_dir}")
            return
    else:
