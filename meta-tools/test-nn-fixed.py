...
from PIL import     
    ...
    if best_mse < 0.05 or final_sim > 0.6:
        ...
        print(f"Found at position: ({final_x}, {final_y})")ort sys
...
def main():
    ...
    # Lower threshold for MSE-based detection
    if final_mse < 0.1 or final_sim > 0.5:
        print("Feature contained")
        ...
        plt.title(f'Found Location (MSE: {final_mse:.4f})')
        plt.axis('off')
        ...
        print(f"Final MSE: {final_mse:.6f} (threshold: 0.1)")
