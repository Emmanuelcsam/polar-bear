...
class DummyLogger:
    ...
    def log_class_init(self, *args, **kwargs):
        pass

    def log_function_entry(self, *args, **kwargs):
        pass

    def log_function_exit(self, *args, **kwargs):
        pass
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SAM, self).__init__(
            self.base_optimizer.param_groups, dict(rho=rho, adaptive=adaptive)
        )
...
class Lookahead(Optimizer):
    ...
    def step(self, closure: Optional[Callable] = None):
        ...
        if closure is not None:
            ...
            # If no closure provided, create a simple one
            def simple_closure():
                return torch.tensor(
                    0.0, device=next(self.optimizer.param_groups[0]["params"]).device
                )

            loss = self.optimizer.step(simple_closure)
    ...
    def state_dict(self):
        ...
        slow_state = {
            ...
            "slow_weights": [[p.cpu() for p in group] for group in self.slow_weights],
        }
...
def create_advanced_optimizer(model: nn.Module, config: Dict) -> Optimizer:
    ...
    param_groups = [
        ...
        {
            "params": [
                p for n, p in model.named_parameters() if "feature_extractor" in n
            ],
            "lr": lr,
        },
        # Segmentation layers - slightly lower
        {
            "params": [p for n, p in model.named_parameters() if "segmentation" in n],
            "lr": lr * 0.5,
        },
        # Reference embeddings - much slower
        {
            "params": [
                p for n, p in model.named_parameters() if "reference_embeddings" in n
            ],
            "lr": lr * 0.1,
        },
        # Decoder layers
        ...
        {
            "params": [p for n, p in model.named_parameters() if "equation" in n],
            "lr": lr * 0.01,
        },
        # Other parameters
        {
    ...
    logger.info(
        f"Created composed Lookahead(SAM(AdamW)) optimizer with {len(param_groups)} parameter groups"
    )
