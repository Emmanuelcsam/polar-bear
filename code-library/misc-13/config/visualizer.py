...
class FiberOpticsVisualizer:
    ...
    def __init__(self, use_interactive: bool = False):
        ...
        if use_interactive:
            try:
                ...
                self.logger.warning("Failed to set interactive backend 'TkAgg', using non-interactive 'Agg'.")
        ...
        # FIX: Colors and colormaps are now loaded from the config file.
        # REASON: The original code had hardcoded color values and colormap names. This ensures that all
        # visualizations generated by this module are consistent with the rest of the application
        # and can be customized centrally in config.yaml.
        self.region_colors = {k: [c / 255.0 for c in v] for k, v in self.config.visualization.segmentation_colors.items()}
        self.anomaly_cmap = plt.get_cmap(self.config.visualization.anomaly_colormap)
    ...
    def visualize_config_overview(self, save_path: Optional[str] = None):
        ...
        # Create pie chart
        colors = plt.cm.tab10(np.linspace(0, 1, len(labels)))
        pie_result = ax5.pie(values, labels=labels, colors=colors,
        ...
        param_keys = list(anomaly_params.keys())
        param_values = list(anomaly_params.values())
        bars = ax8.bar(param_keys, param_values, 
    ...
    def visualize_equation_components(self, results: Dict, save_path: Optional[str] = None):
        ...
        if 'weighted_components' in results['equation_info']:
            ...
            pie_result = ax2.pie(percentages, labels=labels2, 
    ...
    def visualize_config_diff(self, config_dict: Dict, reference_dict: Optional[Dict] = None, 
                             save_path: Optional[str] = None):
        """
    ...
    def generate_config_report(self, save_path: str = "config_report.html"):
        ...
        html_content += """
            <div class="section">
                ...
                <p>Master Equation: S(R) = AÃ—xâ‚ + BÃ—xâ‚‚ + CÃ—xâ‚ƒ + DÃ—xâ‚„ + EÃ—xâ‚…</p>
                <table>
