...
class FiberOpticsDataset(Dataset):
    ...
    def _determine_class_label(self, path: Path) -> int:
        ...
        # Check against region categories if they exist
        if hasattr(self.config, 'REGION_CATEGORIES') and self.config.REGION_CATEGORIES:
            for region, idx in self.class_to_idx.items():
                if folder_name in self.config.REGION_CATEGORIES.get(region, []):
                    return idx
        else:
            # Use simple heuristics for class determination
            folder_lower = folder_name.lower()
            if 'core' in folder_lower or 'good' in folder_lower or 'normal' in folder_lower:
                if 'core' in self.class_to_idx:
                    return self.class_to_idx['core']
            elif 'cladding' in folder_lower:
                if 'cladding' in self.class_to_idx:
                    return self.class_to_idx['cladding']
...
class FiberOpticsDataLoader:
    ...
    def _collect_data_paths(self) -> List[Path]:
        ...
        # Check if legacy REGION_CATEGORIES exists in config
        if hasattr(self.config, 'REGION_CATEGORIES') and hasattr(self.config, 'TENSORIZED_DATA_PATH'):
            ...
            # Use modern config structure
            tensorized_path = Path(self.config.system.tensorized_data_path)
            if tensorized_path.exists():
                for folder_path in tensorized_path.iterdir():
                    if folder_path.is_dir() and any(folder_path.glob("*.pt")):
                        paths.append(folder_path)
                        self.logger.debug(f"Found tensorized folder: {folder_path.name}")
