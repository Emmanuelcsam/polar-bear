...
class FiberSpecifications:
    ...
    cladding_diameter_um: Optional[float] = 125.0  # Diameter of the fiber cladding in micrometers (default).
...
class ZoneDefinition:
    ...
    r_min_factor_or_um: float  # Minimum radius factor (relative to main radius) or absolute radius in µm.
    r_max_factor_or_um: float  # Maximum radius factor (relative to main radius) or absolute radius in µm.
    color_bgr: Tuple[int, int, int]  # BGR color for visualizing this zone.
    max_defect_size_um: Optional[float] = None  # Max allowable defect size in µm (for pass/fail).
    defects_allowed: bool = True  # Whether defects are allowed in this zone.
...
class DetectedZoneInfo:
    ...
    radius_px: float  # Radius in pixels (representing r_max_px for the zone).
    radius_um: Optional[float] = None  # Radius in micrometers (if conversion available).
...
class DefectInfo:
    ...
    confidence_score: float = 0.0  # Confidence score for detection (0.0 to 1.0).
    detection_methods: List[str] = field(default_factory=list)  # Methods that found this defect.
...
class ImageAnalysisStats:
    ...
    processing_time_s: float = 0.0  # Time taken to process the image (seconds).
    status: str = "Pending"  # Pass/Fail/Review status.
    microns_per_pixel: Optional[float] = None  # Calculated µm/px for this image.
...
class ImageResult:
    ...
    fiber_specs_used: FiberSpecifications  # Specs used for this image.
    operating_mode: str  # "PIXEL_ONLY", "MICRON_CALCULATED", or "MICRON_INFERRED".
    detected_zones: Dict[str, DetectedZoneInfo] = field(default_factory=dict)  # Zone info.
    defects: List[DefectInfo] = field(default_factory=list)  # All detected defects.
    stats: ImageAnalysisStats = field(default_factory=ImageAnalysisStats)  # Summary stats.
    annotated_image_path: Optional[Path] = None  # Path to annotated image.
    report_csv_path: Optional[Path] = None  # Path to per-image CSV.
    histogram_path: Optional[Path] = None  # Path to defect histogram.
    ...
    timing_log: Dict[str, float] = field(default_factory=dict)  # Per-step durations.

# --- Configuration Class (from Part 1) ---

@dataclass
class InspectorConfig:
    """Holds all configuration parameters for the fiber inspection process."""
    OUTPUT_DIR_NAME: str = "fiber_inspection_output"  # Directory to save results.
    MIN_DEFECT_AREA_PX: int = 10  # Minimum pixel area to consider a contour a defect.
    PERFORM_CALIBRATION: bool = False  # Whether to attempt system calibration with a target.
    CALIBRATION_IMAGE_PATH: Optional[str] = None  # Path to calibration target image.
    CALIBRATION_DOT_SPACING_UM: float = 10.0  # Known spacing of calibration dots in µm.
    CALIBRATION_FILE_JSON: str = "calibration_data.json"  # File to load/save calibration data.

    # Default zone definitions for PIXEL_ONLY mode (relative factors). Will be scaled if cladding is found.
    DEFAULT_ZONES: List[ZoneDefinition] = field(default_factory=lambda: [
        ZoneDefinition(
            name="core",
            r_min_factor_or_um=0.0,
            r_max_factor_or_um=0.4,  # 50 µm core for 125 µm cladding
            color_bgr=(255, 0, 0),
    ...
    HOUGH_MIN_DIST_FACTOR: float = 0.25
    HOUGH_PARAM1_VALUES: List[int] = field(default_factory=lambda: [70, 100, 130])
    HOUGH_PARAM2_VALUES: List[int] = field(default_factory=lambda: [35, 45, 55])
    HOUGH_MIN_RADIUS_FACTOR: float = 0.1
...
# --- Main Inspector Class (Combines Part 1, Part 2, and new methods for Part 3) ---
...
class FiberInspector:
    ...
    def _get_user_specifications(self):
        ...
        if provide_specs_input == 'y':
            ...
            try:
                core_dia_str = input(
                    f"Enter CORE diameter in microns (e.g., 9, 50, 62.5) (optional, press Enter to skip): "
                ).strip()
                if core_dia_str:
                    self.fiber_specs.core_diameter_um = float(core_dia_str)

                clad_dia_str = input(
                    f"Enter CLADDING diameter in microns (e.g., 125) (default: {self.fiber_specs.cladding_diameter_um}): "
                ).strip()
                if clad_dia_str:
                    self.fiber_specs.cladding_diameter_um = float(clad_dia_str)

                ferrule_dia_str = input(
                    f"Enter FERRULE outer diameter in microns (e.g., 250) (default: {self.fiber_specs.ferrule_diameter_um}): "
                ).strip()
                if ferrule_dia_str:
                        ...
                        try:
                            ...
                            if circles is not None:
                                ...
                                for i in circles[0, :]:
                                    ...
                                    # Compute a simple confidence heuristic
    ...
    def _calculate_pixels_per_micron(self, detected_cladding_radius_px: float) -> Optional[float]:
        """
        Calculates the pixels_per_micron ratio from the detected cladding radius (px)
        and the known cladding diameter in microns.
        """
    ...
    def _create_zone_masks(
        ...
        # Create squared distance map from center
        ...
        for zone_def in self.active_zone_definitions:
            ...
            # **FIXED**: use dist_from_center_sq consistently
            zone_mask_np = (
    ...
    # --- Defect Detection Algorithm Implementations (Part 2) ---
    ...
    def _detect_defects_canny(self, image_gray: np.ndarray, zone_mask: np.ndarray, zone_name: str) -> Optional[np.ndarray]:
        """
        Detects line-like defects using Canny edge detection followed by morphological operations.
        Returns a binary mask of candidate edges within the zone.
        """
    ...
    def _detect_defects_adaptive_thresh(self, image_gray: np.ndarray, zone_mask: np.ndarray, zone_name: str) -> Optional[np.ndarray]:
        ...
        Returns a binary mask within the zone, or None on error.
        """
    ...
    def _combine_defect_masks(self, all_defect_maps: Dict[str, Optional[np.ndarray]], image_shape: Tuple[int, int]) -> np.ndarray:
        """
        Combines all intermediate defect maps into one final binary mask by OR-ing them.
        """
    ...
    def _analyze_defect_contours(
        ...
        """
        Finds contours in combined_mask, computes properties, and returns a list of DefectInfo.
        """
    ...
    def process_single_image(self, image_path: Path) -> ImageResult:
        ...
        Returns an ImageResult object containing everything (zones, defects, stats).
        """
        ...
        for zone_name, zone_info in self.current_image_result.detected_zones.items():
            ...
            all_defect_maps[f"do2mr_{zone_name}"] = self._detect_region_defects_do2mr(
                gray_detect, zone_info.mask, zone_name
            )
            all_defect_maps[f"lei_{zone_name}"] = self._detect_scratches_lei(
                gray_detect, zone_info.mask, zone_name
            )
            all_defect_maps[f"canny_{zone_name}"] = self._detect_defects_canny(
                processed_images.get('gaussian_blurred', gray_detect), zone_info.mask, zone_name
            )
            all_defect_maps[f"adaptive_thresh_{zone_name}"] = self._detect_defects_adaptive_thresh(
                processed_images.get('bilateral_filtered', gray_detect), zone_info.mask, zone_name
            )
        ...
        self.current_image_result.defects = self._analyze_defect_contours(
            final_combined_mask, image_path.name, all_defect_maps
        )
    ...
    def process_image_batch(self, image_paths: List[Path]):
        """
        Processes a batch of images and writes both per-image and batch summary reports.
        """
    ...
    def _save_image_artifacts(self, original_bgr_image: np.ndarray, image_res: ImageResult):
        """
        Saves annotated image, detailed CSV, histogram, and (optionally) intermediate maps.
        """
        ...
        # Intermediate defect maps (optional)
    ...
    def _save_individual_image_report_csv(self, image_res: ImageResult, output_dir: Path):
        """
        Saves a detailed CSV report listing every defect found in a single image.
        """
        ...
        try:
            with open(report_path, mode='w', newline='', encoding='utf-8') as csvfile:
                ...
                for defect in image_res.defects:
                    ...
                    if self.pixels_per_micron and self.pixels_per_micron > 0:
                        cx_um = cx_px / self.pixels_per_micron
                        cy_um = cy_px / self.pixels_per_micron
                    else:
                        cx_um = None
                        cy_um = None
                    ...
                    if self.pixels_per_micron and self.pixels_per_micron > 0:
                        w_um = w_px / self.pixels_per_micron
                        h_um = h_px / self.pixels_per_micron
                    else:
                        w_um = None
                        h_um = None
    ...
    def _generate_annotated_image(self, original_bgr_image: np.ndarray, image_res: ImageResult) -> Optional[np.ndarray]:
        """
        Draws colored circles for each zone and bounding boxes for defects, plus summary text.
        ...
        for zone_name, zone_info in image_res.detected_zones.items():
            ...
            color = zone_def.color_bgr if zone_def else (255, 255, 255)
        ...
        # Overlay text: filename, status, defect count
        cv2.putText(
            annotated_image,
    ...
    def _generate_defect_histogram(self, image_res: ImageResult) -> Optional[plt.Figure]:
        """
        Generates a polar scatter plot showing defect locations (angle vs radius).
        Returns a Figure or None if not enough data.
        """
        ...
        for zone_name, zone_info in image_res.detected_zones.items():
            ...
            line_color = np.array(zone_def.color_bgr)[::-1] / 255.0  # BGR -> RGB normalized
            ax.plot(theta, [r]*360, color=line_color, linestyle='--')
    ...
    def _save_batch_summary_report_csv(self):
        """
        Saves a summary CSV (one row per image) using self.batch_results_summary_list.
        """
