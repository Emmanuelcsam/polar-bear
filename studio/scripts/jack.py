#!/usr/bin/env python3
"""
Ultra-Comprehensive Matrix Anomaly Detection System (v2.0 - Corrected)
======================================================================
...
CORRECTIONS MADE:
- Fixed crash with fewer than 2 reference files
- Fixed crash when no local anomalies are found
- Fixed division by zero in confidence calculations
- Added bounds checking for JSON pixel coordinates
- Renamed misleading persistent homology function
- Improved warning handling
- Added additional error checking throughout
...
- Topological proxy features (connected components analysis)
...
Usage:
1. Provide a folder containing reference JSON files (minimum 2 files)
...
class UltraComprehensiveMatrixAnalyzer:
    ...
    def extract_ultra_comprehensive_features(self, image: np.ndarray) -> Tuple[Dict[str, float], List[str]]:
        ...
        # Extract all feature categories with error handling
        feature_extractors = [
    ...
    def _extract_wavelet_features(self, gray: np.ndarray) -> Dict[str, float]:
        ...
        for wavelet in ['db4', 'haar', 'sym4']:
            try:
                ...
                # Skip if wavelet decomposition fails
                pass
    ...
    def _extract_topological_proxy_features(self, gray: np.ndarray) -> Dict[str, float]:
        """
        ...
        not true persistent homology from TDA.
        """
        features = {}
        
        # Use percentile thresholds for robustness
        ...
        # Track holes (proxy for Betti 1) - using inverted thresholding
        n_holes = []
        for t in thresholds:
            binary = gray < t
            labeled, n = label(binary)
    ...
    def compute_exhaustive_comparison(self, features1: Dict[str, float], features2: Dict[str, float]) -> Dict[str, float]:
        ...
        if len(vec1) == 0 or len(vec2) == 0:
            return self.compute_exhaustive_comparison({}, {})  # Return defaults
        ...
        bins = min(30, len(vec1) // 2)  # Adaptive binning
        if bins > 2:
            ...
            # KL divergence
            kl_div = np.sum(np.where(hist1 > 0, hist1 * np.log((hist1 + 1e-10) / (hist2 + 1e-10)), 0))
            comparison['kl_divergence'] = float(kl_div)
            
            # JS divergence
            m = 0.5 * (hist1 + hist2)
            js_div = 0.5 * np.sum(np.where(hist1 > 0, hist1 * np.log((hist1 + 1e-10) / (m + 1e-10)), 0)) + \
                     0.5 * np.sum(np.where(hist2 > 0, hist2 * np.log((hist2 + 1e-10) / (m + 1e-10)), 0))
        ...
        try:
            ...
            comparison['wasserstein_distance'] = float('inf')
        
        # Structural Similarity (on feature vectors)
        mean1, mean2 = np.mean(vec1), np.mean(vec2)
        comparison['feature_ssim'] = float((2 * mean1 * mean2 + 1e-10) / 
                                          (mean1**2 + mean2**2 + 1e-10))
    ...
    def build_comprehensive_reference_model(self, ref_dir: str):
        ...
        for i in range(len(all_features)):
            for j in range(i + 1, len(all_features)):
                ...
                # Compute anomaly score with safe division
                score = (comp['euclidean_distance'] * 0.2 +
                        comp['manhattan_distance'] * 0.1 +
                        comp['cosine_distance'] * 0.2 +
                        (1 - abs(comp['pearson_correlation'])) * 0.1 +
                        min(comp['kl_divergence'], 10.0) * 0.1 +  # Cap KL divergence
                        comp['js_divergence'] * 0.1 +
                        min(comp['chi_square'], 10.0) * 0.1 +  # Cap chi-square
                        min(comp['wasserstein_distance'], 10.0) * 0.1)  # Cap Wasserstein
        ...
        if len(scores_array) > 0 and not np.all(np.isnan(scores_array)):
            ...
            if len(valid_scores) > 0:
                ...
                print("  ⚠ Warning: All scores are invalid, using default thresholds")
                thresholds = self._get_default_thresholds()
        else:
            print("  ⚠ Warning: No valid comparison scores, using default thresholds")
            thresholds = self._get_default_thresholds()
    ...
    def detect_anomalies_comprehensive(self, test_path: str) -> Optional[Dict[str, Any]]:
        ...
        # Compute Z-scores with protection against zero std
        ...
        # Multiple criteria for anomaly detection with safe thresholds
        is_anomalous = (
        ...
        # Overall confidence with safe division
...
def main():
    ...
    print("\n" + "="*80)
    print("ULTRA-COMPREHENSIVE MATRIX ANOMALY DETECTION SYSTEM (v2.0)".center(80))
    print("="*80)
