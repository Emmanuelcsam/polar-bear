...
class RunTrain(Trainer):
    ...
    and joint multitask training and validation
        ...
        if self.experiment_type == "classify":
            while epoch < max_epoch:
                (iteration,
                 ...
                 best_metrics_valid_class) = self.train_classifier(
                    classifier,
                    iteration,
                    epoch,
            ...
            while epoch < max_epoch:
                ...
                (iteration, losses_train_seg, losses_valid_seg,
                 best_metrics_valid_seg) = self.train_segmenter(
                    segmenter,
                    iteration,
                    epoch,
            ...
            while epoch < max_epoch:
                (iteration, losses_train_seg, losses_valid_seg,
                 losses_train_class, losses_valid_class, best_metrics_valid_class, best_metrics_valid_seg) = \
                    self.train_joint(
                        classifier,
                        segmenter,
                        iteration,
    ...
    def train_classifier(self,
        ...
        if (
            ...
            losses_valid, mean_accuracy = self.valid_classifier(
            ...
            if mean_accuracy > best_metrics_valid:
                ...
                utils.save_checkpoint(
                    ckpt_name='best_metric_classifier',
                    ckpt_dir=self.ckpt_dir,
                    model=classifier,
                    optimizer=self.optimizer_class,
                    iteration=iteration,
    ...
    def train_segmenter(
        ...
        if (
            ...
            if mean_dice_val > best_metrics_valid:
                ...
                utils.save_checkpoint(
                    ckpt_name='best_metric_segmenter',
                    ckpt_dir=self.ckpt_dir,
                    model=segmenter,
                    optimizer=self.optimizer_seg,
                    iteration=iteration,
    ...
    def train_joint(
        ...
        for batch in epoch_iterator:
            ...
            logit_map = segmenter(img)
            total_loss_seg, multi_loss, binary_loss = self.compute_seg_loss(
                logit_map,
                mask,
                LP,
                multi_seg_weight=multi_seg_weight,
        ...
        if (
            ...
            (losses_valid_seg, losses_valid_class,
             mean_dice_val, mean_accuracy) = self.valid_joint(
                segmenter,
                classifier,
                binary_seg_weight=binary_seg_weight,
                multi_seg_weight=multi_seg_weight,
    ...
    def valid_joint(
        ...
        """
        Validation loop for our multitask framework .
        ...
        for batch in epoch_iterator_val:
            ...
            metrics_valid_seg['binary_valid_loss_seg'].append(binary_loss.item())
            metrics_valid_seg['multi_valid_loss_seg'].append(multi_loss.item())
