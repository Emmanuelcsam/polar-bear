...
def process_image(image: np.ndarray, 
    ...
    try:
        ...
        # 5. Create Mask using the Equation of a Circle
        h, w = gray.shape
        ...
        # 6. Isolate the Core
        # Use the original image (color or gray) to preserve original appearance
        if len(image.shape) == 3:
            # Create a 3-channel mask for color images
            core_mask_color = cv2.cvtColor(core_mask, cv2.COLOR_GRAY2BGR)
            isolated_core = cv2.bitwise_and(image, core_mask_color)
        else:
            isolated_core = cv2.bitwise_and(gray, gray, mask=core_mask)
            
        # 7. Final Touches (Highlighting & Cropping)
        result = isolated_core
        if highlight_boundary:
            # Ensure we can draw color on the result
            if len(result.shape) == 2:
                result = cv2.cvtColor(result, cv2.COLOR_GRAY2BGR)
            cv2.circle(result, (int(round(cx)), int(round(cy))), int(round(r_core)), (0, 255, 0), 1)

        if crop_output:
            coords = np.argwhere(core_mask > 0)
            if coords.size > 0:
        ...
        print(f"Error in veridian_core_detector: {e}")
